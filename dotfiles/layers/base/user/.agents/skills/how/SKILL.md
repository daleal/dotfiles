...
Spawn all explorers concurrently in the foreground and wait for every explorer to finish before continuing to synthesis. If the available tooling requires background execution for concurrency, launch them together, continue only independent local research while they run, and then wait for all pending explorers before synthesizing or sending a final response. Never end the turn or present conclusions while an explorer is still running. Each explorer gets the same base prompt from `references/explorer-prompt.md` plus a specific exploration angle naming its slice. Each explorer should:
...
Spawn a single Task subagent that explores and explains in one pass. The agent does its own exploration (Glob, Grep, Read) and writes the explanation directly. Read `references/explainer-prompt.md` for the communication style and output format. Same structure, just no explorer findings as input.

Proceed to Step 4.

### Step 3. Synthesize (complex questions only)

Once all explorers return, spawn a single Task subagent to synthesize their findings into one coherent explanation. The explainer gets all explorers' findings and writes the human-facing explanation (output format below). Read `references/explainer-prompt.md` for the full prompt template. The explainer reconciles overlapping findings, resolves contradictions, and weaves the slices into a unified picture.
...
After the explanation is complete, spawn an architectural critic. Read `references/critic-prompt.md` for the prompt template. The critic gets:
