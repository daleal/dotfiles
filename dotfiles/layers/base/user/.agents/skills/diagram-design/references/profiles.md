...
Named profiles let one Diagram Design install serve several clients without repeatedly editing `references/style-guide.md`. A profile is a complete style guide stored in that skill installation's `profiles/` directory.
...
- **Profile library:** `profiles/`
- **Profile:** `profiles/<slug>.md`
- **Working copy:** `references/style-guide.md`
...
Resolve all bundled paths relative to the Diagram Design skill root, as required by the Agent Skills specification. Resolve `<project-root>` only for the project marker. Profiles stay with the skill installation; the optional marker selects one for the project.
...
Each file is the full body of `references/style-guide.md` with one metadata comment prepended:
...
`profiles/default.md` is the recovery copy of the current package's pristine shipped `references/style-guide.md`.

Before onboarding overwrites a pristine working copy, and again on the first `save` or `load`, check for `profiles/default.md`. If it is absent:
...
3. **Bash:** from the skill root, create the library with `mkdir -p profiles`.
...
- For `profile: <slug>`, resolve only `profiles/<slug>.md`, run the structural check, and read that effective guide directly for this generation. Do not copy it over the installed working copy.
...
Marker-first direct reads are what make two parallel workspaces with different clients safe. A generation resolved through a marker must leave `references/style-guide.md` byte-for-byte unchanged.
...
5. **Bash:** from the skill root, run `mkdir -p profiles`. If the directory cannot be created or written, report the failure and offer to paste/save the full profile manually; do not claim success.
...
4. If a syntactically valid project marker exists—even one naming a missing profile—explain that marker-first projects do not use the shared working copy and ask permission to replace the marker with exactly `profile: <slug>`. On approval, **Write** the marker and do not touch `references/style-guide.md`.
...
1. Inspect `profiles/` without creating it. If absent or empty, say no saved profiles exist; mention that `default` is created on first save/load.
...
- **Managed update replaced the working copy:** reload a named project profile explicitly, or rely on the project marker.
- **Profile library is unwritable:** show the intended canonical path and offer a manual full-file paste. Do not claim success.
- **Install directory is unwritable:** do not claim a copy-over load succeeded. Offer the project-marker flow, which reads the project-local profile directly.
